    ...
    <div className="p-4 md:p-6 space-y-5 pb-24 md:pb-6 max-w-2xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Shop News</h1>
            <p className="text-xs text-gray-500">Post updates, offers, and announcements — requires admin approval</p>
          </div>
        </div>
        <button
          type="button"
      ...
      {showForm && (
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5 space-y-4">
          <h2 className="text-sm font-bold text-blue-900">{editing ? 'Edit Post' : 'New Post'}</h2>
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                placeholder="Write your update, offer, or announcement here…"
                className="mt-1 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
            </div>
            <div>
          ...
          {(!title.trim() || !body.trim()) && (
            <p className="text-xs text-red-600 flex items-center gap-1"><AlertCircle className="h-3.5 w-3.5" />Title and content are required</p>
          )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 py-16 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">Create your first shop news post</p>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {post.status === 'REJECTED' && post.reviewNote && (
                      <p className="mt-1 text-xs text-red-600 bg-red-50 rounded-lg px-2 py-1">
                        Rejection note: {post.reviewNote}
                      </p>
                    )}
                    {isExpanded && (
