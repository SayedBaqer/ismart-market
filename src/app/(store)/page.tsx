  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3 gap-1">
        {p.category && (
          <span className="text-[10px] font-semibold uppercase tracking-wider text-blue-500 leading-none">{p.category.name}</span>
        )}
        <p className="text-sm font-semibold text-gray-900 line-clamp-2 leading-snug flex-1 min-h-[2.5rem]">{p.name}</p>
    ...
    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none sm:grid sm:grid-cols-4 sm:overflow-visible -mx-4 px-4 sm:mx-0 sm:px-0">
      {products.map((p, i) => (
        <div key={p.id} className="w-40 flex-none sm:w-auto">
          <ProductCard p={p} currency={currency} rank={i + 1} />
        </div>
      ))}
    </div>
  )
  ...
  const [ads, categories, topSellers, onSale, topViewed, newArrivals, topShops, currency, storeName] =
    await Promise.all([
      ...
      prisma.product.findMany({
        ...
        take: 8,
        select: { id: true, slug: true, name: true, price: true, comparePrice: true, images: true, category: { select: { name: true } } },
      }).catch(() => []),
        ...
        <div className="relative bg-gradient-to-br from-blue-700 to-blue-900 text-white overflow-hidden">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-14 sm:py-20 flex flex-col sm:flex-row items-center gap-8">
            <div className="flex-1 text-center sm:text-left">
              <p className="text-blue-200 text-sm font-semibold uppercase tracking-widest mb-2">Welcome to</p>
              <h1 className="text-4xl sm:text-5xl font-black leading-tight mb-4">{name}</h1>
              <p className="text-blue-100 text-base mb-6 max-w-md">Discover the best products from top shops in one place.</p>
              <Link href="/products"
          ...
          <div className="mx-auto max-w-7xl px-4 py-4">
            <div className="flex gap-2.5 overflow-x-auto scrollbar-none pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap sm:justify-start">
              <Link href="/products"
                ...
                <span className="text-[11px] font-semibold text-gray-600 whitespace-nowrap">All</span>
              </Link>
              {categories.map((cat) => (
                <Link key={cat.id} href={`/products?category=${cat.slug}`}
                  ...
                  <div className="h-14 w-14 rounded-2xl overflow-hidden bg-gray-100 shadow-sm group-hover:scale-105 transition-transform">
                    {(cat as { imageUrl?: string | null }).imageUrl ? (
                      <Image src={(cat as { imageUrl: string }).imageUrl} alt={cat.name} width={56} height={56} className="object-cover h-full w-full" />
                    ) : (
                      <div className="h-full w-full flex items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100">
                        <span className="text-lg font-black text-blue-400">{cat.name[0]}</span>
                      </div>
                    )}
                  </div>
                  <span className="text-[11px] font-semibold text-gray-600 whitespace-nowrap max-w-[56px] text-center leading-tight line-clamp-1">{cat.name}</span>
                </Link>
              ))}
            </div>
          </div>
        ...
        {topSellers.filter(p => Number(p.price) > 0).length > 0 && (
          <section>
            <SectionHeader
              ...
              href="/products"
            />
            <ProductRow products={topSellers.filter(p => Number(p.price) > 0)} currency={curr} />
          </section>
        )}

        {/* ── Hot Deals ───────────────────────────────────────────────── */}
        {hotDeals.length > 0 && (
          <section className="-mx-4 px-4 sm:mx-0 sm:px-0 bg-gradient-to-r from-red-50 via-orange-50 to-yellow-50 rounded-3xl py-6 px-4 sm:px-6">
            <SectionHeader
              icon={<Flame className="h-5 w-5 text-red-500" />}
              title="Hot Deals"
              href="/products"
            />
            <ProductRow products={hotDeals} currency={curr} />
          </section>
        )}
        ...
        {topViewed.filter(p => Number(p.price) > 0).length > 0 && (
          <section>
            <SectionHeader
              ...
              href="/products"
            />
            <ProductRow products={topViewed.filter(p => Number(p.price) > 0)} currency={curr} />
          </section>
        )}
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-none sm:grid sm:grid-cols-8 sm:overflow-visible -mx-4 px-4 sm:mx-0 sm:px-0">
              {topShops.map((shop) => (
                <Link key={shop.id} href={`/shops/${shop.slug}`}
                  ...
                  <span className="text-[11px] font-semibold text-gray-600 text-center line-clamp-1 w-full">{shop.name}</span>
                  <span className="text-[10px] text-gray-400">{shop._count.products} items</span>
                </Link>
              ))}
            </div>
          </section>
        ...
        {newArrivals.filter(p => Number(p.price) > 0).length > 0 && (
          <section>
            <SectionHeader
              ...
              href="/products"
            />
            <ProductRow products={newArrivals.filter(p => Number(p.price) > 0)} currency={curr} />
          </section>
        )}

        {/* ── Browse All CTA ───────────────────────────────────────────── */}
        <div className="rounded-3xl bg-gradient-to-r from-blue-600 to-blue-700 p-8 text-center text-white">
          <h3 className="text-xl font-black mb-2">Discover More</h3>
          <p className="text-blue-100 text-sm mb-5">Thousands of products waiting for you</p>
          <Link href="/products"
